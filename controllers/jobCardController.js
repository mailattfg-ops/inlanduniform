...
function isolateGarmentMeasurements(dynamicData, itemName, productName, productTypeName) {
  ...
  const bottomKeywords = ["pant", "pants", "trouser", "trousers", "suiting", "bottom", "skirt", "salwar", "short", "shorts", "track pant"];
  ...
  const entries = Object.entries(dynamicData).filter(([k]) => !k.startsWith("_"));
  ...
  if (nestedEntries.length > 0) {
    ...
    if (matchedGroup) {
      ...
      Object.entries(matchedGroup).forEach(([k, v]) => {
        if (k !== "strategy" && k !== "chart_id" && !k.startsWith("_") && v !== undefined && v !== null && v !== "") {
          isolated[k] = v;
        }
      });
  ...
  entries.forEach(([mKey, mVal]) => {
    ...
    if (isCardTop && !isCardBottom) {
      if (topMetricNames.has(mKeyLower) || topKeywords.some((kw) => mKeyLower.includes(kw))) {
        isolated[mKey] = mVal;
      }
    } else if (isCardBottom && !isCardTop) {
      if (bottomMetricNames.has(mKeyLower) || bottomKeywords.some((kw) => mKeyLower.includes(kw))) {
        isolated[mKey] = mVal;
      }
    } else {
      isolated[mKey] = mVal;
...
async function checkOrderMeasurementReadiness(order_id) {
  try {
    const { data: orderRec } = await supabase
      .from("orders")
      .select("id, quotation_id, quotations(id, organization_id, metrics_summary)")
      .eq("id", order_id)
      .maybeSingle();

    if (!orderRec || !orderRec.quotations) {
      return { isMeasurementsPending: false, pendingCount: 0, totalMembers: 0, approvedMembers: 0 };
    }

    const orgId = orderRec.quotations.organization_id;
    if (!orgId) {
      return { isMeasurementsPending: false, pendingCount: 0, totalMembers: 0, approvedMembers: 0, quotationId: orderRec.quotation_id };
    }
    ...
    measurements?.forEach((m) => {
      ...
      if (!memberStatus[m.member_id] || m.status === "Approved" || m.status === "Completed") {
        memberStatus[m.member_id] = m.status;
      }
    });

    const approvedMembers = Object.keys(memberStatus).filter(
      (mid) => memberStatus[mid] === "Approved" || memberStatus[mid] === "Completed"
    ).length;
    ...
    console.error("[JobCardController] checkOrderMeasurementReadiness error:", e.message);
    return { isMeasurementsPending: false, pendingCount: 0, totalMembers: 0, approvedMembers: 0 };
  }
}

// Helper: Check physical fabric readiness from factory stock (PRD M9.8)
async function checkFabricReadiness(fabric_id, fabric_name, quantity, main_fabric_meters) {
  try {
    if (!fabric_id && !fabric_name) {
      return { isFabricDepleted: false, availableMeters: 0, requiredMeters: 0, fabricHoldReason: null };
    }

    let fQuery = supabase.from("fabrics").select("id, name, code, quantity, low_stock_threshold");
    ...
    const requiredMeters = parseFloat(((quantity || 1) * consumptionPerPc).toFixed(2));
    ...
    console.error("[JobCardController] checkFabricReadiness error:", err.message);
    return { isFabricDepleted: false, availableMeters: 0, requiredMeters: 0, fabricHoldReason: null };
  }
}
...
async function syncJobCardsForOrder(order_id) {
  try {
    ...
    const { data: jobCards } = await supabase
      .from("job_cards")
      .select("id, status, po_handler_action, hold_reason, size_breakdown, quantity")
    ...
    for (const jc of jobCards) {
      ...
      const fReadiness = await checkFabricReadiness(fId, fName, jc.quantity, cRate);
      ...
      if (readiness.isMeasurementsPending && isFabricDepleted) {
        ...
        newStatus = jc.status.startsWith("Held") ? "Pending PO Handler" : jc.status;
      }
      ...
      const updatedBreakdown = {
        ...
        measurement_readiness: readiness.isMeasurementsPending ? "Awaiting Measurements" : "Ready",
      ...
      if (jc.status.startsWith("Held") || newStatus.startsWith("Held")) {
        ...
        updatePayload.po_handler_action = newStatus.startsWith("Held")
          ? "Hold"
          : (jc.po_handler_action === "Hold" ? "Pending" : jc.po_handler_action);
      }
    ...
    if (readiness.quotationId && readiness.totalMembers > 0) {
      ...
      const updatedMetrics = {
        ...
        missing: Math.max(0, readiness.totalMembers - readiness.approvedMembers),
      };
      await supabase
        .from("quotations")
...
async function resolveAllFabricsForJobCard(jobCard, sizeBreakdown = null) {
  ...
  if (jobCard?.item_id) {
    try {
      ...
      console.warn("[JobCardController] Quotation item fabric fetch caught:", qErr.message);
    }
  }

  // 1. Main fabric attributes
  let mainId = sb.fabric_id || sb.main_fabric_id || qItemSb?.fabric_id || qItemSb?.main_fabric_id || null;
  let mainName = sb.fabric_name || sb.main_fabric_name || qItemSb?.fabric_name || qItemSb?.main_fabric_name || null;
  let mainMeters = parseFloat(sb.main_fabric_meters || sb.consumption || qItemSb?.main_fabric_meters || 1.25);
  ...
  let att1Id = sb.attachment_fabric1_id || qItemSb?.attachment_fabric1_id || null;
  let att1Name = sb.attachment_fabric1_name || qItemSb?.attachment_fabric1_name || null;
  let att1Meters = (sb.attachment_fabric1_meters !== undefined && sb.attachment_fabric1_meters !== null)
    ? parseFloat(sb.attachment_fabric1_meters)
    : (qItemSb?.attachment_fabric1_meters !== undefined && qItemSb?.attachment_fabric1_meters !== null)
    ? parseFloat(qItemSb.attachment_fabric1_meters)
    : null;
  let att1Code = sb.attachment_fabric1_code || qItemSb?.attachment_fabric1_code || null;
  let att1Shade = sb.attachment_fabric1_shade || qItemSb?.attachment_fabric1_shade || null;

  // 3. Attachment fabric 2 attributes
  let att2Id = sb.attachment_fabric2_id || qItemSb?.attachment_fabric2_id || null;
  let att2Name = sb.attachment_fabric2_name || qItemSb?.attachment_fabric2_name || null;
  let att2Meters = (sb.attachment_fabric2_meters !== undefined && sb.attachment_fabric2_meters !== null)
    ? parseFloat(sb.attachment_fabric2_meters)
    : (qItemSb?.attachment_fabric2_meters !== undefined && qItemSb?.attachment_fabric2_meters !== null)
    ? parseFloat(qItemSb.attachment_fabric2_meters)
    : null;
  let att2Code = sb.attachment_fabric2_code || qItemSb?.attachment_fabric2_code || null;
  let att2Shade = sb.attachment_fabric2_shade || qItemSb?.attachment_fabric2_shade || null;
  ...
  const missingNames = [mainName, att1Name, att2Name].filter((n) => n && !fabMap[n.toLowerCase().trim()]);
  ...
  // Resolve Main
  const matchedMain = (mainId && fabMap[String(mainId)]) || (mainName && fabMap[mainName.toLowerCase().trim()]);
  const finalMainCode = matchedMain?.code || mainCode || (mainId ? `FAB-${mainId}` : "FAB-STD");
  const finalMainName = matchedMain?.name || mainName || "Standard Production Fabric";
  const finalMainShade = matchedMain?.shade || mainShade || null;
  const finalMainLength = isNaN(mainMeters) || mainMeters <= 0 ? 1.25 : mainMeters;
  ...
  if (att1Id || att1Name || (att1Meters !== null && att1Meters > 0)) {
    const matchedAtt1 = (att1Id && fabMap[String(att1Id)]) || (att1Name && fabMap[att1Name.toLowerCase().trim()]);
    const finalAtt1Code = matchedAtt1?.code || att1Code || (att1Id ? `FAB-${att1Id}` : "ATT1-STD");
    const finalAtt1Name = matchedAtt1?.name || att1Name || "Attachment Fabric 1";
    const finalAtt1Shade = matchedAtt1?.shade || att1Shade || null;
    const finalAtt1Length = att1Meters !== null && !isNaN(att1Meters) ? att1Meters : 0;
  ...
  if (att2Id || att2Name || (att2Meters !== null && att2Meters > 0)) {
    const matchedAtt2 = (att2Id && fabMap[String(att2Id)]) || (att2Name && fabMap[att2Name.toLowerCase().trim()]);
    const finalAtt2Code = matchedAtt2?.code || att2Code || (att2Id ? `FAB-${att2Id}` : "ATT2-STD");
    const finalAtt2Name = matchedAtt2?.name || att2Name || "Attachment Fabric 2";
    const finalAtt2Shade = matchedAtt2?.shade || att2Shade || null;
    const finalAtt2Length = att2Meters !== null && !isNaN(att2Meters) ? att2Meters : 0;
...
async function generateChildJobCardsForJobCard(jobCard, options = {}) {
  try {
    ...
    if (existing && existing.length > 0 && options.force) {
      await supabase.from("child_job_cards").delete().eq("job_card_id", jobCardId);
    }

    // Fetch order details with quotation & organization
    const { data: orderData } = await supabase
      .from("orders")
      .select("id, order_no, quotation_id, quotations(id, organization_id, organizations(id, name, customer_code))")
      .eq("id", orderId)
      .maybeSingle();

    const orgId = orderData?.quotations?.organization_id;
    const orgCode = (orderData?.quotations?.organizations?.customer_code || "ORG").toUpperCase().replace(/[^A-Z0-9]/g, "");
    const cleanOrderNo = (orderData?.order_no || `ORD${orderId}`).toUpperCase().replace(/[^A-Z0-9]/g, "");
    const cleanItemName = (jobCard.item_name || "ITEM").toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 5);
    const jcNoDigits = (jobCard.job_card_no || `JC${jobCardId}`).replace(/[^0-9]/g, "").slice(-4) || String(jobCardId);
    ...
    if (!targetDeptId && jobCard.item_id) {
      try {
        ...
        console.warn("[JobCardController] Could not resolve item department:", qErr.message);
      }
    }
    ...
    if (orgId) {
      let mQuery = supabase
        .from("registry_members")
        .select(`
          id, full_name, admission_no, gender, department_id,
          measurements(id, dynamic_data, suggested_size, notes, status, recorded_at)
        `)
        .eq("organization_id", orgId);

      // Strict Department Isolation (PRD M11.1 & M11.2)
      if (targetDeptId) {
        mQuery = mQuery.eq("department_id", targetDeptId);
      }

      const { data: fetchedMembers } = await mQuery.order("id", { ascending: true });

      if (fetchedMembers && fetchedMembers.length > 0) {
        deptMembers = fetchedMembers.map((m) => {
          const validMeas = (m.measurements || []).sort(
            (a, b) => new Date(b.recorded_at) - new Date(a.recorded_at)
          )[0];
          return {
            ...m,
            latest_measurement: validMeas || null,
          };
        });
      }
    }
    ...
    if (targetProductId) {
      try {
        ...
        if (prodData) {
          ...
          targetProductTypeName = prodData.product_types?.name || targetProductTypeName;
        }
      } catch (pErr) {
        console.warn("[JobCardController] Could not fetch product details:", pErr.message);
      }
    }

    // Resolve all fabric specifications (Main Fabric, Attachment Fabric 1, Attachment Fabric 2)
    const fabricSpecObj = await resolveAllFabricsForJobCard(jobCard, sizeBreakdown);

    const isCustomMode = deptMembers.length > 0;
    ...
    if (isCustomMode) {
      ...
      const basePiecesPerMember = Math.max(1, Math.floor(totalQty / numMembers));
      ...
      deptMembers.forEach((member, mIdx) => {
        ...
        const admCode = (member.admission_no || `M${member.id}`).replace(/[^A-Z0-9]/gi, "").toUpperCase();
        ...
        const isolatedMeas = isolateGarmentMeasurements(rawDyn, jobCard.item_name, targetProductName, targetProductTypeName);

        let pieceSize = member.latest_measurement?.suggested_size || "Custom";
        if (isolatedMeas.selected_size && typeof isolatedMeas.selected_size === "object") {
          const szVals = Object.values(isolatedMeas.selected_size).filter(Boolean);
        ...
        for (let pIdx = 1; pIdx <= memberQty && seq <= totalQty; pIdx++) {
          ...
          piecesToInsert.push({
            ...
            custom_measurements: {
              ...isolatedMeas,
              _target_dress: targetProductName || targetProductTypeName || "Garment",
            ...
            notes: memberQty > 1 ? `Piece ${pIdx} of ${memberQty}` : member.latest_measurement?.notes || null,
    ...
    if (piecesToInsert.length > 0) {
      try {
        ...
        if (insertErr) {
          console.warn("[JobCardController] child_job_cards DB insert skipped (table pending in Supabase):", insertErr.message);
          return { success: true, count: piecesToInsert.length, isCustomMode, fallback: true, pieces: piecesToInsert };
        }
        return { success: true, count: inserted?.length || piecesToInsert.length, isCustomMode, pieces: inserted || piecesToInsert };
      } catch (dbErr) {
        console.warn("[JobCardController] child_job_cards DB insert caught:", dbErr.message);
        return { success: true, count: piecesToInsert.length, isCustomMode, fallback: true, pieces: piecesToInsert };
      }
    }
    ...
    console.error("[JobCardController] generateChildJobCardsForJobCard error:", err.message);
...
exports.createJobCardFromOrder = async (req, res) => {
  try {
    ...
    let consumptionPerPc = parseFloat(size_breakdown?.main_fabric_meters || 1.25);
    ...
    if (!resolvedFabricId && item_id) {
      ...
      if (qItem?.size_breakdown) {
        ...
        resolvedFabricName = resolvedFabricName || qItem.size_breakdown.fabric_name;
        consumptionPerPc = parseFloat(qItem.size_breakdown.main_fabric_meters || consumptionPerPc);
      }
    }
    ...
    const enrichedSizeBreakdown = {
      ...
      measurement_readiness: isMeasurementsPending ? "Awaiting Measurements" : "Ready",
      material_readiness: isFabricDepleted ? "Awaiting PO Fabric" : "Ready",
    ...
    try {
      ...
      console.warn("[JobCardController] Child cards generation warning:", cjcErr.message);
    }
...
exports.poHandlerAction = async (req, res) => {
  try {
    ...
    if (action === "Accept") {
      ...
      if (currentCard) {
        const readiness = await checkOrderMeasurementReadiness(currentCard.order_id);
        if (readiness.isMeasurementsPending) {
          return res.status(400).json({
    ...
    try {
      ...
      console.warn("[JobCardController] PO Handler activity log caught:", logErr.message);
    }
...
async function checkAndUpdateJobCardCompletion(jobCardId, user = null) {
  ...
  try {
    ...
    try {
      ...
      console.warn("[JobCardController] child_job_cards check warning:", cErr.message);
    }
    ...
    if (subCards && subCards.length > 0) {
      ...
      const allBatchesReady = subCards.every((sc) => (sc.stage || "").toLowerCase() === "ready");
      if (allBatchesReady) {
        isCompleted = true;
      }
      ...
      const allPiecesReady = childCards.every(
        (c) => (c.stage || "").toLowerCase() === "ready" || (c.status || "").toLowerCase() === "ready" || (c.status || "").toLowerCase() === "completed"
      );
      if (allPiecesReady) {
        isCompleted = true;
      }
    ...
    if (isCompleted) {
      if (jc.status !== "Ready") {
        ...
        try {
          await supabase.from("record_activity_logs").insert([
            {
              ...
              performed_by_name: user?.fullName || user?.email || "System Automation",
              details: {
          ...
          console.warn("[JobCardController] Completion activity log caught:", actErr.message);
        }
      }
      ...
      if (jc.order_id) {
        ...
        if (orderJcs && orderJcs.length > 0) {
          ...
          if (allOrderJcsReady) {
            ...
            if (currentOrder && currentOrder.status !== "Ready for Dispatch" && currentOrder.status !== "Dispatched" && currentOrder.status !== "Delivered") {
              await supabase
                .from("orders")
                .update({
                ...
                await supabase.from("record_activity_logs").insert([
                  {
                    ...
                    performed_by: toSafeInt(user?.id),
                    performed_by_name: user?.fullName || user?.email || "Production Floor",
                    details: {
                      order_no: currentOrder.order_no,
                      total_job_cards: orderJcs.length,
                      message: "All Job Cards for this sales order have completed production. Order advanced to Ready for Dispatch.",
                    },
                  },
                ]);
              } catch (oActErr) {
                console.warn("[JobCardController] Order dispatch log caught:", oActErr.message);
              }
            }
          }
        }
    ...
    console.error("[JobCardController] checkAndUpdateJobCardCompletion error:", compErr.message);
  }
}
...
exports.listJobCards = async (req, res) => {
  try {
    ...
    const buildQuery = () => {
      let q = supabase
        .from("job_cards")
        .select("*, orders(order_no, quotations(quotation_no, organization_id, organizations(name))), fabric_consumption_logs(id, fabric_name, required_meters, safety_margin_meters, issued_meters, returned_meters, cut_piece_batch_no, created_at)")
      ...
      if (status) {
        ...
        if (statusList.includes("Accepted") && statusList.includes("In Production") && !statusList.includes("Ready")) {
    ...
    const uniqueOrderIds = [...new Set((data || []).map((j) => j.order_id).filter(Boolean))];
    ...
    const enriched = (finalData || []).map((jc) => {
      ...
      const totalFabricIssued = logs.reduce((sum, l) => sum + (parseFloat(l.issued_meters) || 0), 0);
      return {
        ...jc,
        ...
        measurement_readiness:
          ...
          (jc.status === "Held (Awaiting Measurements)" ? "Awaiting Measurements" : "Ready"),
      };
    });
...
exports.createSubJobCard = async (req, res) => {
  try {
    const { job_card_id, batch_size, target_date, assigned_to, selected_child_ids } = req.body;
    ...
    const requestedBatchSize = allocatedPieces.length > 0 
      ? allocatedPieces.length 
      : parseInt(batch_size, 10);

    if (isNaN(requestedBatchSize) || requestedBatchSize <= 0) {
      return res.status(400).json({ error: "Batch size must be a positive number greater than 0." });
    }
    ...
    const alreadyAllocated = (existingSubCards || []).reduce((acc, sc) => acc + (parseInt(sc.batch_size, 10) || 0), 0);
    ...
    if (initialRes.error) {
      if (initialRes.error.message?.includes("size_distribution") || initialRes.error.code === "PGRST204") {
        console.warn("[JobCardController] size_distribution column not in sub_job_cards schema cache. Inserting without size_distribution column.");
    ...
    if (allocatedPieces.length > 0 && subCard) {
      try {
        ...
        console.warn("[JobCardController] child_job_cards link notice:", cjcErr.message);
      }
    }
    ...
    try {
      ...
      console.error("[JobCardController] Sub-job card activity log failed:", logErr.message);
    }
...
exports.updateSubJobCardStage = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("[JobCardController] Child card stage update warning:", e.message);
    }
...
exports.listSubJobCards = async (req, res) => {
  try {
    ...
    let query = supabase
      .from("sub_job_cards")
      .select("*, job_cards(id, job_card_no, item_name, quantity, status, orders(order_no))")
      .order("created_at", { ascending: false });
...
exports.deleteSubJobCard = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: `Batch ${subCard.sub_card_no} deleted successfully.` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.getRequiredFabricsForJobCard = async (req, res) => {
  try {
    ...
    if (jcErr || !jc) return res.status(404).json({ error: "Job Card not found" });

    // Fetch warehouse fabrics
    const { data: stockFabrics } = await supabase
      .from("fabrics")
      .select("id, name, code, quantity, shade, brand_name, low_stock_threshold")
    ...
    const findStockFabric = (fId, fName, fShade) => {
      ...
      if (fName) {
        ...
        const byName = stockFabrics.find(f => f.name?.toLowerCase().trim() === cleanName);
        if (byName) return byName;
        const byPartial = stockFabrics.find(f => f.name?.toLowerCase().includes(cleanName) || cleanName.includes(f.name?.toLowerCase()));
        if (byPartial) return byPartial;
      }
      if (fShade) {
        const cleanShade = String(fShade).toLowerCase().trim();
        const byShade = stockFabrics.find(f => f.shade?.toLowerCase().trim() === cleanShade);
    ...
    const nestedProducts = (Array.isArray(sizeBreakdown.products) && sizeBreakdown.products.length > 0)
      ? sizeBreakdown.products
      : (Array.isArray(qItemBreakdown?.products) && qItemBreakdown.products.length > 0)
    ...
    if (nestedProducts && nestedProducts.length > 0) {
      for (const p of nestedProducts) {
        ...
        productList.push({
          product_id: safePId,
          product_name: p.product_name || p.name || p.product_type_name || jc.item_name || 'Garment',
          product_type_name: p.product_type_name || '',
          quantity: pQty,
          fabric_id: p.fabric_id || p.main_fabric_id || p.size_breakdown?.fabric_id || null,
          fabric_name: p.fabric_name || p.main_fabric_name || p.size_breakdown?.fabric_name || null,
          consumption: parseFloat(p.main_fabric_meters || p.consumption || p.size_breakdown?.main_fabric_meters) || 1.25,
          attachment_fabric1_id: p.attachment_fabric1_id || p.size_breakdown?.attachment_fabric1_id || null,
          attachment_fabric1_name: p.attachment_fabric1_name || p.size_breakdown?.attachment_fabric1_name || null,
          attachment_fabric1_meters: parseFloat(p.attachment_fabric1_meters || p.size_breakdown?.attachment_fabric1_meters || 0),
          attachment_fabric2_id: p.attachment_fabric2_id || p.size_breakdown?.attachment_fabric2_id || null,
          attachment_fabric2_name: p.attachment_fabric2_name || p.size_breakdown?.attachment_fabric2_name || null,
          attachment_fabric2_meters: parseFloat(p.attachment_fabric2_meters || p.size_breakdown?.attachment_fabric2_meters || 0)
        });
      }
    } else {
      // Single product / garment definition
      const safePId = toSafeInt(sizeBreakdown.product_id) || toSafeInt(qItemBreakdown?.product_id) || null;
      productList.push({
        product_id: safePId,
        product_name: jc.item_name || sizeBreakdown.product_name || qItemBreakdown?.product_name || qItemProductType || 'Garment Item',
        product_type_name: qItemProductType || jc.item_name || '',
        quantity: totalQty,
        fabric_id: sizeBreakdown.fabric_id || sizeBreakdown.main_fabric_id || qItemBreakdown?.fabric_id || null,
        fabric_name: sizeBreakdown.fabric_name || sizeBreakdown.main_fabric_name || qItemBreakdown?.fabric_name || null,
        consumption: parseFloat(sizeBreakdown.main_fabric_meters || qItemBreakdown?.main_fabric_meters) || 1.25,
        attachment_fabric1_id: sizeBreakdown.attachment_fabric1_id || qItemBreakdown?.attachment_fabric1_id || null,
        attachment_fabric1_name: sizeBreakdown.attachment_fabric1_name || qItemBreakdown?.attachment_fabric1_name || null,
        attachment_fabric1_meters: parseFloat(sizeBreakdown.attachment_fabric1_meters || qItemBreakdown?.attachment_fabric1_meters || 0),
        attachment_fabric2_id: sizeBreakdown.attachment_fabric2_id || qItemBreakdown?.attachment_fabric2_id || null,
        attachment_fabric2_name: sizeBreakdown.attachment_fabric2_name || qItemBreakdown?.attachment_fabric2_name || null,
        attachment_fabric2_meters: parseFloat(sizeBreakdown.attachment_fabric2_meters || qItemBreakdown?.attachment_fabric2_meters || 0)
      });
    }

    // Check catalog products table for any missing fabric specs - strictly only valid numeric IDs
    const prodIdsToFetch = productList.map(p => toSafeInt(p.product_id)).filter(Boolean);
    let catalogMap = {};
    if (prodIdsToFetch.length > 0) {
      try {
        const { data: catData } = await supabase
          .from("products")
          .select("id, name, main_fabric, attachment_fabric1, attachment_fabric2, main_fabric_id, attachment_fabric1_id, attachment_fabric2_id")
          .in("id", prodIdsToFetch);
        if (catData) {
          catData.forEach(cp => { catalogMap[cp.id] = cp; });
        }
      } catch (catErr) {
        console.warn("[JobCardController] Products catalog lookup caught:", catErr.message);
      }
    }
    ...
    for (const prod of productList) {
      ...
      const mFabName = prod.fabric_name || catProd?.main_fabric || `${prod.product_name} Production Fabric`;
      ...
      const mSafetyMargin = parseFloat((mReqMeters * 0.10).toFixed(2));
      const mTotalIssue = parseFloat((mReqMeters + mSafetyMargin).toFixed(2));

      const stockRollId = matchedMain?.id || mFabId || (stockFabrics?.[0]?.id || null);
      const originalStock = parseFloat(matchedMain?.quantity || stockFabrics?.[0]?.quantity || 0);
      ...
      requiredFabrics.push({
        ...
        role: productList.length > 1 ? `${prod.product_name} - Main Fabric` : "Main Fabric",
        fabric_key: `${prod.product_name}_main_${requiredFabrics.length}`,
        fabric_id: stockRollId,
        fabric_name: matchedMain?.name || mFabName || stockFabrics?.[0]?.name || "Standard Production Fabric",
      ...
      const att1Id = prod.attachment_fabric1_id || catProd?.attachment_fabric1_id;
      const att1Name = prod.attachment_fabric1_name || catProd?.attachment_fabric1;
      const att1Meters = parseFloat(prod.attachment_fabric1_meters || catProd?.attachment_fabric1_meters || 0);
      ...
      if ((att1Id || att1Name || att1Meters > 0) && att1Meters > 0) {
        ...
        const a1SafetyMargin = parseFloat((a1ReqMeters * 0.10).toFixed(2));
        const a1TotalIssue = parseFloat((a1ReqMeters + a1SafetyMargin).toFixed(2));
        ...
        if (a1RollId) runningStockDeductions[a1RollId] = a1PriorDeduction + a1TotalIssue;

        requiredFabrics.push({
          product_name: prod.product_name,
          role: productList.length > 1 ? `${prod.product_name} - Attachment 1 (Collar / Lining)` : "Attachment Fabric 1 (Collar / Lining)",
      ...
      const att2Id = prod.attachment_fabric2_id || catProd?.attachment_fabric2_id;
      const att2Name = prod.attachment_fabric2_name || catProd?.attachment_fabric2;
      const att2Meters = parseFloat(prod.attachment_fabric2_meters || catProd?.attachment_fabric2_meters || 0);
      ...
      if ((att2Id || att2Name || att2Meters > 0) && att2Meters > 0) {
        ...
        const a2SafetyMargin = parseFloat((a2ReqMeters * 0.10).toFixed(2));
        const a2TotalIssue = parseFloat((a2ReqMeters + a2SafetyMargin).toFixed(2));
        ...
        if (a2RollId) runningStockDeductions[a2RollId] = a2PriorDeduction + a2TotalIssue;

        requiredFabrics.push({
          product_name: prod.product_name,
          role: productList.length > 1 ? `${prod.product_name} - Attachment 2 (Pockets / Trims)` : "Attachment Fabric 2 (Pockets / Trims)",
    ...
    const totalMetersAllFabrics = requiredFabrics.reduce((sum, f) => sum + f.total_issue_meters, 0);
    ...
    console.error("[JobCardController] getRequiredFabricsForJobCard error:", err.message);
    res.status(500).json({ error: err.message });
  }
};
...
exports.logFabricConsumption = async (req, res) => {
  try {
    ...
    if (existingLogs && existingLogs.length > 0) {
      const totalIssued = existingLogs.reduce((acc, l) => acc + (parseFloat(l.issued_meters) || 0), 0);
      const issueDate = new Date(existingLogs[0].created_at).toLocaleDateString('en-IN', {
    ...
    if (Array.isArray(items) && items.length > 0) {
      ...
      return res.status(400).json({ error: "No fabric items provided to issue." });
    }
    ...
    for (const item of itemsToProcess) {
      ...
      if (isNaN(reqMeters) || reqMeters <= 0) {
        return res.status(400).json({ error: `Invalid required meters for fabric ${item.fabric_name || 'item'}.` });
      }
      ...
      stockDeductions.push({
        ...
        safetyMargin,
        issuedMeters,
        returnedMeters: parseFloat(item.returned_meters || returned_meters || 0)
      });
    }

    // Execute stock deduction and logging
    for (const alloc of stockDeductions) {
      const { item, fabricRecord, reqMeters, safetyMargin, issuedMeters, returnedMeters: retM } = alloc;
      ...
      try {
          ...
          .insert([
            {
              ...
              sub_job_card_id: sub_job_card_id ? toSafeInt(sub_job_card_id) : null,
              fabric_id: fabricRecord?.id ? String(fabricRecord.id) : (item.fabric_id ? String(item.fabric_id) : null),
              fabric_name: fabricRecord?.name || item.fabric_name || "Standard Production Fabric",
        ...
        if (
          ...
          console.warn("[JobCardController] fabric_consumption_logs fabric_id bigint type mismatch, retrying with sanitized integer/null:", insertErr.message);
            ...
            .insert([
              {
                job_card_id: safeJobCardId,
                sub_job_card_id: sub_job_card_id ? toSafeInt(sub_job_card_id) : null,
                fabric_id: toSafeInt(fabricRecord?.id) || toSafeInt(item.fabric_id) || null,
                fabric_name: fabricRecord?.name || item.fabric_name || "Standard Production Fabric",
                required_meters: reqMeters,
                safety_margin_meters: safetyMargin,
                issued_meters: issuedMeters,
      ...
      if (fabricRecord) {
        try {
          ...
          const newStock = Math.max(0, parseFloat(((fabricRecord.quantity || 0) - netDeduction).toFixed(2)));
          await supabase
            .from("fabrics")
            .update({ quantity: newStock, updated_at: new Date().toISOString() })
            .eq("id", fabricRecord.id);
        } catch (stockErr) {
          console.error("[JobCardController] Fabric stock deduction error:", stockErr.message);
        }
      }
      ...
      try {
        ...
        console.error("[JobCardController] Fabric activity log warning:", logErr.message);
      }
    }
    ...
    } else if (parentJobCard.status === "Held (Awaiting Measurements & PO Fabric)") {
      updatedStatus = "Held (Awaiting Measurements)";
      updatedHoldReason = updatedHoldReason?.replace(/Fabric.*?depleted.*?PO fabric arrives\.\s*\|?\s*/gi, "") || updatedHoldReason;
    }
    ...
    console.error("[JobCardController] logFabricConsumption error:", err.message);
    res.status(500).json({ error: err.message });
  }
};
...
exports.getEligibleOrdersForJobCards = async (req, res) => {
  try {
    ...
    // Only return orders that have quotation items
    const eligible = (data || []).filter(
      (o) => o.quotations?.quotation_items?.length > 0,
    );
    res.json(eligible);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
async function generateInMemoryPiecesForJobCard(jobCardId) {
  try {
    const { data: jc } = await supabase
      .from("job_cards")
      .select("*, orders(id, order_no, quotation_id, quotations(id, organization_id, organizations(id, name, customer_code)))")
      .eq("id", jobCardId)
      .maybeSingle();
    ...
    const orgCode = (jc.orders?.quotations?.organizations?.customer_code || "ORG").toUpperCase().replace(/[^A-Z0-9]/g, "");
    const cleanOrderNo = (jc.orders?.order_no || `ORD${jc.order_id}`).toUpperCase().replace(/[^A-Z0-9]/g, "");
    const cleanItemName = (jc.item_name || "ITEM").toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 5);
    const jcNoDigits = (jc.job_card_no || `JC${jobCardId}`).replace(/[^0-9]/g, "").slice(-4) || String(jobCardId);
    ...
    if (!targetDeptId && jc.item_id) {
      try {
        ...
        console.warn("[JobCardController] Could not resolve item department in fallback:", qErr.message);
      }
    }
    ...
    if (isCustomMode) {
      ...
      deptMembers.forEach((member, mIdx) => {
        ...
        // Isolate measurements strictly for this specific dress (e.g. T-Shirt only or Pants only)
        const rawDyn = member.latest_measurement?.dynamic_data || {};
        const isolatedMeas = isolateGarmentMeasurements(rawDyn, jc.item_name, targetProductName, targetProductTypeName);
    ...
    console.error("[JobCardController] generateInMemoryPiecesForJobCard error:", err.message);
    return [];
  }
}
...
exports.getChildJobCards = async (req, res) => {
  try {
    ...
    // Fetch parent job card
    const { data: jc } = await supabase.from("job_cards").select("*").eq("id", id).maybeSingle();

    const resolvedFabrics = await resolveAllFabricsForJobCard(jc, jc?.size_breakdown);
    ...
    const enrichPiece = (p) => {
      ...
      const fabCode = p.fabric_code || fMeta.code || mainMeta.code || resolvedFabrics.code;
      const fabName = p.fabric_name || fMeta.name || mainMeta.name || resolvedFabrics.name;
      const fabLength = p.fabric_length || p.fabric_meters || fMeta.length || mainMeta.length || resolvedFabrics.length;
      const fabShade = p.fabric_shade || fMeta.shade || mainMeta.shade || resolvedFabrics.shade;

      const att1Name = p.attachment1_name || att1Meta?.name || resolvedFabrics.attachment1?.name || null;
      const att1Code = p.attachment1_code || att1Meta?.code || resolvedFabrics.attachment1?.code || null;
      const att1Length = p.attachment1_length || p.attachment1_meters || att1Meta?.length || resolvedFabrics.attachment1?.length || null;
      const att1Shade = p.attachment1_shade || att1Meta?.shade || resolvedFabrics.attachment1?.shade || null;

      const att2Name = p.attachment2_name || att2Meta?.name || resolvedFabrics.attachment2?.name || null;
      const att2Code = p.attachment2_code || att2Meta?.code || resolvedFabrics.attachment2?.code || null;
      const att2Length = p.attachment2_length || p.attachment2_meters || att2Meta?.length || resolvedFabrics.attachment2?.length || null;
      const att2Shade = p.attachment2_shade || att2Meta?.shade || resolvedFabrics.attachment2?.shade || null;

      const allFabrics = (Array.isArray(p.fabrics) && p.fabrics.length > 0)
        ? p.fabrics
    ...
    if (error) {
      console.warn("[JobCardController] child_job_cards query noticed:", error.message);
    ...
    if (data.some((p) => p.item_type === "custom" && (!p.custom_measurements?._target_dress && !p.custom_measurements?._garment))) {
      needsHeal = true;
    }

    if (data.some((p) => p.barcode && (p.barcode.length > 14 || p.barcode.startsWith("BRC-")))) {
      needsHeal = true;
    }
    ...
    for (const p of data) {
      if (p.member_id) {
        if (!memberBarcodeMap[p.member_id]) memberBarcodeMap[p.member_id] = new Set();
    ...
    if (needsHeal) {
      try {
        ...
        const genResult = await generateChildJobCardsForJobCard(jc, { force: true });
        if (genResult.pieces && genResult.pieces.length > 0) {
          return res.json(genResult.pieces.map(enrichPiece));
        }
      } catch (healErr) {
    ...
    console.error("[JobCardController] getChildJobCards caught error:", err.message);
    const fallbackPieces = await generateInMemoryPiecesForJobCard(req.params.id);
...
exports.generateChildCardsEndpoint = async (req, res) => {
  try {
    ...
    if (piecesErr || !pieces || pieces.length === 0) {
      ...
      return res.json({ success: true, count: fallback.length, pieces: fallback });
    }
...
exports.scanChildBarcode = async (req, res) => {
  try {
    ...
    let piece = matchedPieces && matchedPieces.length > 0 ? matchedPieces[0] : null;
    ...
    if (!piece) {
      ...
      if (match) {
        ...
        if (jcRows && jcRows.length > 0) {
          const fallbackPieces = await generateInMemoryPiecesForJobCard(jcRows[0].id);
          const found = fallbackPieces.find((p) => p.barcode === cleanBarcode);
          if (found) {
            const { data: fullJc } = await supabase
              .from("job_cards")
              .select("id, job_card_no, item_name, design_number, status, size_breakdown, order_id, orders(id, order_no, quotations(organization_id, organizations(name)))")
              .eq("id", jcRows[0].id)
              .single();
            piece = {
              ...found,
    ...
    if (!piece) return res.status(404).json({ error: `No garment piece found matching barcode: ${cleanBarcode}` });

    const fMeta = piece.custom_measurements?._fabric || {};
    const jcSb = piece.job_cards?.size_breakdown || {};
    const resolvedFabrics = await resolveAllFabricsForJobCard(piece.job_cards, jcSb);

    const mainMeta = fMeta.main || {};
    const att1Meta = fMeta.attachment1 || null;
    const att2Meta = fMeta.attachment2 || null;

    const fabCode = piece.fabric_code || fMeta.code || mainMeta.code || resolvedFabrics.code;
    const fabName = piece.fabric_name || fMeta.name || mainMeta.name || resolvedFabrics.name;
    const fabLength = piece.fabric_length || piece.fabric_meters || fMeta.length || mainMeta.length || resolvedFabrics.length;
    const fabShade = piece.fabric_shade || fMeta.shade || mainMeta.shade || resolvedFabrics.shade;

    const att1Name = piece.attachment1_name || att1Meta?.name || resolvedFabrics.attachment1?.name || null;
    const att1Code = piece.attachment1_code || att1Meta?.code || resolvedFabrics.attachment1?.code || null;
    const att1Length = piece.attachment1_length || piece.attachment1_meters || att1Meta?.length || resolvedFabrics.attachment1?.length || null;
    const att1Shade = piece.attachment1_shade || att1Meta?.shade || resolvedFabrics.attachment1?.shade || null;

    const att2Name = piece.attachment2_name || att2Meta?.name || resolvedFabrics.attachment2?.name || null;
    const att2Code = piece.attachment2_code || att2Meta?.code || resolvedFabrics.attachment2?.code || null;
    const att2Length = piece.attachment2_length || piece.attachment2_meters || att2Meta?.length || resolvedFabrics.attachment2?.length || null;
    const att2Shade = piece.attachment2_shade || att2Meta?.shade || resolvedFabrics.attachment2?.shade || null;

    const allFabrics = (Array.isArray(piece.fabrics) && piece.fabrics.length > 0)
      ? piece.fabrics
      : (Array.isArray(fMeta.all) && fMeta.all.length > 0)
      ? fMeta.all
      : resolvedFabrics.all;
  ...
  try {
    ...
    if (data.sub_job_card_id) {
      try {
        ...
        console.warn("[JobCardController] Sibling pieces check warning:", sibErr.message);
      }
    }
